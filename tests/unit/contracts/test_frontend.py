...
def project(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("apmx.runtime.utils.find_runtime_binary", lambda name: str(binary))
    return tmp_path
...
def test_plan_reads_without_native_probe_install_config_or_writes(
    ...
    monkeypatch.setattr(
        "apmx.runtime.copilot_runtime.CopilotRuntime.build_contract_request", forbidden
    )
...
def test_missing_executable_is_halted(project: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("apmx.runtime.utils.find_runtime_binary", lambda name: None)
    with pytest.raises(ContractError) as error:
