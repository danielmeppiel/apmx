...
from apmx.models.dependency.types import GitReferenceType, ResolvedReference
...
def test_cli_harness_admission_belongs_to_frontend(
    ...
    monkeypatch.setattr("apmx.runtime.utils.find_runtime_binary", resolve_binary)
...
def test_no_policy_gate_precedes_remote_acquisition(
    ...
    monkeypatch.setattr("apmx.cli.prepare_contract_source", acquire)
...
def test_remote_plan_never_initializes_downloader(
    ...
    monkeypatch.setattr("apmx.install.git_source.download_git", download)
    result = CliRunner().invoke(
...
def test_remote_acquisition_preserves_reference_object(
    ...
    monkeypatch.setattr(
        "apmx.install.git_source.download_git", _transport(downloader)
    )
    with contract_source.prepare_contract_source(
        "org/repo/jobs#v1",
        "job.contract.md",
...
def test_installed_remote_plan_is_exact_and_read_only(
    ...
    monkeypatch.setattr("apmx.install.git_source.download_git", downloader)
    with contract_source.prepare_contract_source(
...
def _transport(downloader):
    def acquire(reference, target):
        info = downloader.download_package(reference, target)
        return reference, info.resolved_reference.resolved_commit, target
    return acquire
