...
**The v0.3.0 prebuilt instructions are staged; the new downloads are not
...
| Route | What you get |
| --- | --- |
| [Prebuilt v0.3.0](#install-a-prebuilt-archive) | The intended default once published: download, verify and extract a complete native bundle. No Python runtime installation for APMX itself. |
...
**Publication gate:** do not run these download blocks until the new v0.3.0
...
| Machine | Archive |
| --- | --- |
| macOS, Apple Silicon | `apmx-0.3.0-macos-arm64.tar.gz` |
| macOS, Intel | `apmx-0.3.0-macos-x86_64.tar.gz` |
| Linux, x86-64 | `apmx-0.3.0-linux-x86_64.tar.gz` |
| Linux, ARM64 | `apmx-0.3.0-linux-arm64.tar.gz` |
| Windows, x86-64 | `apmx-0.3.0-windows-x86_64.zip` |
...
Expect the command under `APMX_NATIVE_ROOT` and version `0.3.0`. This PATH applies
...
Expect `apmx.exe` in the extracted directory, version `0.3.0`, native
...
v0.3.0 distribution must pass the corrected third-party notice gates before it
