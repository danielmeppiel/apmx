...
``apmx.commands._helpers._atomic_write`` (kept as an alias for
backward compatibility with existing tests) and
``apmx.compilation.output_writer`` route through here.
"""
...
def normalize_crlf_to_lf(data: str) -> str:
    ...
    Mirrors :func:`apmx.utils.normalization._normalize_line_endings` at the
