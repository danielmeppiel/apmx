...
def is_github_hostname(hostname: str | None) -> bool:
    ...
    :meth:`~apmx.core.auth.AuthResolver.classify_host` so that parse-time
...
def set_authorization_header_git_env(env: dict[str, str], scheme: str, credential: str) -> None:
    """Delegate Git authorization replacement to the child-env owner."""
    from apmx.utils.git_env import set_git_authorization_header
...
def parse_artifactory_path(path_segments: list) -> tuple:
    ...
    proxy -- is determined by :func:`apmx.install.artifactory_resolver.\
