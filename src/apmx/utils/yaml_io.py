...
All YAML file operations in apmx should use these helpers to ensure
...
class _BoundedYAMLHandler(_FrontmatterYAMLHandler):
    ...
    apmx (the primitive parsers, the install + audit-replay integrators,
...
def dump_yaml(
    ...
    *,
    ...
    :func:`apmx.utils.atomic_io.write_text_lf`, the codebase's single
    LF-write policy). Several callers rewrite ``apm.yml`` INSIDE an
    installed package tree that
    :func:`apmx.utils.content_hash.compute_package_hash` hashes raw
