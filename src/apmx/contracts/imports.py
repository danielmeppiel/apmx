...
def _package(raw: bytes, root: Path) -> tuple[APMPackage, dict[str, Any]]:
    try:
        ...
        if not isinstance(data, dict):
            # Decoded manifest types are schema values, not Python argument types.
...
def _context_files(root: Path, limits: ContractLimits) -> tuple[tuple[Path, str, str], ...]:
    ...
    for name in files:
        ...
        try:
            ...
            raise ContractError(
                "Invalid selected context frontmatter.", code="invalid_import"
            ) from exc
        ...
        if kind == "instruction" and document.metadata.get("applyTo") not in (
            ...
            raise ContractError(
                "Scoped instruction activation is unsupported.", code="unsupported_import"
            )
        if any(
            ...
            raise ContractError(
                "Selected context requires unsupported activation.", code="unsupported_import"
            )
        ...
        if kind == "skill":
            if (
                not document.metadata.get("name")
                or not isinstance(document.metadata.get("description"), str)
    ...
    *,
    ...
    for locked in lock.dependencies.values():
        ...
        if locked.name and manifest_name and locked.name != manifest_name:
            raise ContractError(
                "Installed package name differs from its lock.", code="import_drift"
            )
        aliases = {
            locked.repo_url,
    ...
    for name in contract.imports:
        ...
        if locked.target_subset and not {"all", "copilot", "agent-skills"}.intersection(
            ...
            raise ContractError(
                "Selected package targets exclude this context adapter.", code="unsupported_import"
            )
        if locked.skill_subset and "*" not in locked.skill_subset:
            available = {path.parent.name for path, kind, _ in contexts if kind == "skill"}
            if not set(locked.skill_subset).issubset(available):
                raise ContractError(
                    "Locked skill selection is not installed.", code="missing_import"
                )
            contexts = tuple(
                item
        ...
        if locked.depth == 1:
            ...
            if len(declarations) != 1 or detect_ref_change(declarations[0], locked):
                raise ContractError(
                    "Manifest and installed lock reference differ.", code="import_drift"
                )
            ...
            raise ContractError(
                "Transitive context lacks its parent lock identity.", code="import_drift"
            )
        if not contexts:
        ...
        if locked.source == "local":
            ...
            if declared_origin.is_absolute():
                ...
                if has_symlink_component(Path(declared_origin.anchor), declared_origin):
                    raise ContractError(
                        "Selected local package contains a symlink.", code="import_source"
                    )
        ...
        if locked.source != "local":
            if not locked.resolved_commit or not locked.content_hash:
                raise ContractError(
                    "Git import needs a locked commit and package hash.", code="import_drift"
                )
