...
interpreter injects ``truststore`` at startup with no ``apmx`` dependency and
...
def _child_bootstrap_dir() -> str | None:
    ...
    try:
        if getattr(sys, "frozen", False):
            ...
            if not base:
                return None
            candidate = Path(base) / "apmx" / "core" / _CHILD_SHIM_DIRNAME
        else:
...
def ensure_child_tls_bootstrap(venv_path: str | os.PathLike[str]) -> bool:
    ...
    ``truststore`` at startup -- no ``apmx`` dependency, no ``PYTHONPATH``
